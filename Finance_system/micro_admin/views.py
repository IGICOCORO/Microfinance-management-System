...
from .mixins import BranchAccessRequiredMixin, BranchManagerRequiredMixin
...
def create_branch_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return JsonResponse({
                "error": False,
                "success_url": reverse('branchprofile', kwargs={"pk": branch.id})
            })
        else:
...
def update_branch_view(request, pk):
    ...
    if not request.user.is_admin:
        return HttpResponseRedirect(reverse("viewbranch"))
    if request.method == 'POST':
        form = BranchForm(request.POST, instance=branch)
        if form.is_valid():
            branch = form.save()
            return JsonResponse({
                "error": False, "success_url": reverse('branchprofile', kwargs={'pk': branch.id})
            })
        else:
...
def update_client_view(request, pk):
    ...
    if request.method == 'POST':
        form = ClientForm(request.POST, user=request.user,
                          client=client_obj, instance=client_obj)
        if form.is_valid():
            client = form.save()
            return JsonResponse({
                "error": False,
...
def updateclientprofileview(request, pk):
    ...
    signature = str(client_obj.signature).split(
        '/')[-1] if client_obj.signature else None
        ...
        if (loans and savings_account) or savings_account or loans:
            if loans and savings_account:
                if savings_account and savings_account.savings_balance != 0:
                    raise Http404(
                        "Oops! Member is involved in savings, Unable to delete.")
                # elif loans and loans.count() != loans.filter(status='Closed').count():
                    ...
                    raise Http404(
                        "Oops! Member is involved in loan, Unable to delete.")
                else:
...
def create_user_view(request):
    ...
    permissions = Permission.objects.filter(
        content_type_id=contenttype, codename__in=["branch_manager"])
    form = UserForm()
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return JsonResponse({
                "error": False,
                "success_url": reverse('userprofile', kwargs={"pk": user.id})
            })
        else:
            return JsonResponse({"error": True, "errors": form.errors})
...
def group_profile_view(request, group_id):
    ...
    group_mettings = GroupMeetings.objects.filter(
        group_id=group_obj.id).order_by('-id')
...
def group_remove_members_view(request, group_id, client_id):
    ...
    group_loan_accounts = LoanAccount.objects.filter(
        group=group, group__account_number=group.account_number, client__isnull=True)
    group_savings_account = SavingsAccount.objects.filter(
        group=group, group__account_number=group.account_number, client__isnull=True).last()
    client_savings_account = SavingsAccount.objects.filter(
        client=client).last()
    count = 0
    if (group_loan_accounts and (group_savings_account and client_savings_account)) or\
        ...
        if group_loan_accounts and (client_savings_account and client_savings_account):
            if client_savings_account.savings_balance != 0:
                raise Http404(
                    "Oops! Unable to delete this Member, Savings Account Not yet Closed.")
...
def fixed_deposits_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            fixed_deposit.save()
            url = reverse('micro_admin:clientfixeddepositsprofile', kwargs={
...
def view_particular_client_fixed_deposits(request, client_id):
    client = get_object_or_404(Client, id=client_id)
    client_fixed_deposits = FixedDeposits.objects.filter(
