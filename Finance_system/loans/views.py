...
def client_loan_account(request, pk):
    ...
    no_of_repayments_completed = int(
...
def client_loan_ledger_view(request, client_id, loanaccount_id):
    ...
    context = {'client': client, 'loanaccount': loanaccount,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(group.clients.all()):
                ...
                loan_account.total_loan_balance = d(
                    d(loan_account.loan_amount))
                loan_account.save()
...
def group_loan_account(request, loanaccount_id):
    ...
    context['total_loan_paid'] = total_loan_amount_repaid + \
...
def change_loan_account_status(request, pk):
    ...
    data["success_url"] = reverse(
        'loans:clientloanaccount', kwargs={"pk": loan_object.id})
...
def issue_loan(request, loanaccount_id):
    ...
    if loan_account.group:
        ...
        url = reverse("loans:clientloanaccount",
