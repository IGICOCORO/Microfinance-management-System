            ...
            if form.client:
                ...
                if default_group:
                    ...
                    for loan in group_accounts_filter:
                        ...
                        if payments:
                            group_loan_has_payments.append(
                                loan.group_loan_account.id)
                            ...
                            if form.cleaned_data.get("loaninterest_amount") ==\
                                ...
                                if loan_account.interest_type == "Flat":
                                    ...
                                    interest_charged = ((int(loan_account.loan_repayment_every) * ((loan_account.total_loan_balance) * (
                                        (loan_account.annual_interest_rate) / 12)) / 100))
                                loan_account.interest_charged = (
                                    balance_interest + interest_charged)
                                    ...
                                    interest_charged = (
                                        int(loan_account.loan_repayment_every) * (
                                            ((loan_account.total_loan_balance) * (
                                                (loan_account.annual_interest_rate) / 12)) / 100))
                                    balance_interest = d(
                                        loan_account.interest_charged) - d(form.cleaned_data.get("loaninterest_amount"))
                                    loan_account.interest_charged = d(
                                        balance_interest + interest_charged)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.savings_account:
                ...
                if form.cleaned_data.get('recurring_deposit_account_no'):
                    recurring_deposit_account = RecurringDeposits.objects.filter(
                        reccuring_deposit_number=form.cleaned_data.get(
                ...
                if form.cleaned_data.get('fixed_deposit_account_no'):
                    fixed_deposit_account = FixedDeposits.objects.filter(
                        fixed_deposit_number=form.cleaned_data.get(
            ...
            if form.cleaned_data.get("insurance_amount"):
                client.insurance_amount += (
                    form.cleaned_data.get("insurance_amount", 0))
            ...
            if form.cleaned_data.get("loanprinciple_amount") \
               or (form.cleaned_data.get("loaninterest_amount")) != 0:
                if form.loan_account:
                    ...
                    group_loan_account = loan_valid(
                        form, group_member_loan_account)
            if form.cleaned_data.get("sharecapital_amount") or\
                ...
                if form.cleaned_data.get("sharecapital_amount"):
                    receipt.sharecapital_amount = form.cleaned_data.get(
                        "sharecapital_amount")
                if form.cleaned_data.get("entrancefee_amount"):
                    receipt.entrancefee_amount = form.cleaned_data.get(
                        "entrancefee_amount")
                if form.cleaned_data.get("membershipfee_amount"):
                    receipt.membershipfee_amount = form.cleaned_data.get(
                        "membershipfee_amount")
                if form.cleaned_data.get("bookfee_amount"):
                    receipt.bookfee_amount = form.cleaned_data.get(
                        "bookfee_amount")
                if form.cleaned_data.get("loanprocessingfee_amount"):
                    receipt.loanprocessingfee_amount = form.cleaned_data.get(
                ...
                if form.cleaned_data.get("savingsdeposit_thrift_amount"):
                    receipt.savingsdeposit_thrift_amount = form.cleaned_data.get(
                ...
                if form.savings_account:
                    ...
                    if form.cleaned_data.get("fixeddeposit_amount"):
                        receipt.fixeddeposit_amount = form.cleaned_data.get(
                            "fixeddeposit_amount")
                    ...
                    if form.cleaned_data.get("recurringdeposit_amount"):
                        receipt.recurringdeposit_amount = form.cleaned_data.get(
                            "recurringdeposit_amount")
                if form.cleaned_data.get("insurance_amount"):
                    receipt.insurance_amount = form.cleaned_data.get(
                        "insurance_amount")
                if form.cleaned_data.get("loanprinciple_amount"):
                    receipt.loanprinciple_amount = form.cleaned_data.get(
                ...
                if form.cleaned_data.get("loaninterest_amount") != 0:
                    receipt.loaninterest_amount = form.cleaned_data.get(
                        "loaninterest_amount")
                    if form.loan_account:
                        receipt.member_loan_account = loan_account
                    if form.group_loan_account:
                ...
                if form.group_loan_account:
                    ...
                    group_member_loan_account = GroupMemberLoanAccount.objects.filter(
                        group_loan_account=form.group_loan_account)
                    if group_member_loan_account:
                        ...
                        if count == group_member_loan_account.count():
                            group_loan = LoanAccount.objects.get(
                                id=form.group_loan_account.id)
        ...
        if form.is_valid():
            ...
            if pay_slip.loan_account:
                loan_account = LoanAccount.objects.get(
                    id=pay_slip.loan_account.id)
                loan_account.loan_issued_date = pay_slip.date
                loan_account.save()
                group_members = GroupMemberLoanAccount.objects.filter(
                    group_loan_account=pay_slip.loan_account)
                if group_members:
    ...
    if group_name and group_account_number:
        ...
        if group_filter:
            ...
            loan_accounts = LoanAccount.objects.filter(
                group=group, status='Approved', loan_issued_date__isnull=True)
            # if loan_accounts:
            #     for account in loan_accounts:
            #         loan_accounts_data[account.id] = account.account_no
            loan_accounts_data = loan_accounts.values_list(
                "id", "account_no", "loan_amount")
        else:
...
def get_member_loan_accounts(request):
    ...
    if client_name and client_account_number:
        ...
        if member_filter:
            ...
            loan_accounts = LoanAccount.objects.filter(
                client=client, status='Approved', loan_issued_date__isnull=True)
            # if loan_accounts:
            #     for account in loan_accounts:
            #         loan_accounts_data[account.id] = account.account_no
            loan_accounts_data = loan_accounts.values_list(
...
def get_fixed_deposit_paid_accounts_view(request):
    ...
    if form.is_valid():
        ...
        interest_charged = (fixed_deposit.fixed_deposit_amount *
                            fixed_deposit.fixed_deposit_interest_rate) / (d(year_days) * 100)
...
def get_recurring_deposit_paid_accounts_view(request):
    ...
    if form.is_valid():
        ...
        year_days = 366 if calendar.isleap(current_date.year) else 365
        interest_charged = (recurring_deposit_amount *
                            recurring_deposit.recurring_deposit_interest_rate) / (d(year_days) * 100)
        days_to_calculate = (
