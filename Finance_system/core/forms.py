...
class ClientLoanAccountsForm(forms.Form):
    ...
    def clean(self):
        ...
        if not(self.cleaned_data.get("name") and self.cleaned_data.get("account_number")):
            ...
            errors.append(
                "Please provide both  member first name, account number")
    ...
    def clean(self):
        ...
        if self.cleaned_data.get("loan_account_no"):
            self.loan_account = LoanAccount.objects.filter(
                account_no=self.cleaned_data.get("loan_account_no")).last()
        elif self.cleaned_data.get("group_loan_account_no"):
            self.group_loan_account = LoanAccount.objects.filter(
                account_no=self.cleaned_data.get("group_loan_account_no")).last()
            self.loan_account = GroupMemberLoanAccount.objects.get(
...
class GetFixedDepositsForm(forms.Form):
    ...
    def clean(self):
        ...
        if not self.fixed_deposit_account:
            errors = self._errors.setdefault("message1", ErrorList())
            errors.append(
                "No Fixed Deposit Accounts found with given a/c number")
            raise forms.ValidationError(errors)
...
class GetRecurringDepositsForm(forms.Form):
    ...
    recurring_deposit_account_no = forms.CharField(
...
class ReceiptForm(forms.ModelForm):
    ...
    def clean_receipt_number(self):
        ...
        is_receipt_number_exist = Receipts.objects.filter(
            receipt_number=receipt_number)
        if is_receipt_number_exist:
            raise forms.ValidationError(
                "Receipts with this Receipt number already exists.")
                    ...
                    if not ((self.cleaned_data.get("loanprinciple_amount", 0) or d('0.00')) <= (loan_account.total_loan_balance)):
                        ...
                        if (self.cleaned_data.get("loaninterest_amount", 0) or d('0.00')) > (loan_account.interest_charged):
                            ...
                            errors = self._errors.setdefault(
                                "message1", ErrorList())
                            errors.append(
                                "Entered principle amount is greater than demand amount.")
                            raise forms.ValidationError(errors)
                            ...
                            loan_account.total_loan_paid = (
                                loan_account.total_loan_amount_repaid + loan_account.total_interest_repaid)
                            if self.cleaned_data.get("loanprinciple_amount", 0):
    ...
    def clean(self):
        ...
        if name and account_number:
            self.client = Client.objects.filter(
                first_name__iexact=name, account_number=account_number).last()
            if not self.client:
                errors = self._errors.setdefault("message1", ErrorList())
                errors.append(
                    "No Client exists with this First Name and Account number.")
        ...
        if loan_account_no:
            self.loan_account = LoanAccount.objects.filter(
                client=self.client, account_no=loan_account_no).last()
            if not self.loan_account:
                errors = self._errors.setdefault("message1", ErrorList())
                errors.append(
        ...
        if group_name and group_account_number:
            self.group = Group.objects.filter(
                name__iexact=group_name, clients=self.client, account_number=group_account_number).last()
            if not self.group:
                errors = self._errors.setdefault("message1", ErrorList())
                errors.append(
        ...
        if self.cleaned_data.get("loanprinciple_amount") or self.cleaned_data.get("loaninterest_amount") != 0:
            if self.loan_account:
                ...
                self.group_member_loan_account = GroupMemberLoanAccount.objects.filter(
                    group_loan_account=self.group_loan_account, client=self.client).first()
                if self.group_member_loan_account:
                    if self.group_member_loan_account.status == "Approved":
                        ...
                        self.group_loan_account.total_loan_balance -= self.cleaned_data.get(
        ...
        if self.cleaned_data.get('fixed_deposit_account_no'):
            fixed_deposit_account = FixedDeposits.objects.filter(
                fixed_deposit_number=self.cleaned_data.get(
                    'fixed_deposit_account_no')
            ).first()
            if fixed_deposit_account:
                savings_account = SavingsAccount.objects.filter(
                    client=fixed_deposit_account.client)
                if savings_account:
                            ...
                            if self.cleaned_data.get('fixeddeposit_amount') >= 0:
                                if d(fixed_deposit_account.fixed_deposit_amount) != d(self.cleaned_data.get('fixeddeposit_amount')):
                                    errors = self._errors.setdefault(
                                        'message1', ErrorList())
                                    errors.append(
                                        'Entered fixed amount is not equal to the actual amount.')
                                    raise forms.ValidationError(errors)
                            else:
                                errors = self._errors.setdefault(
                                    'fixeddeposit_amount', ErrorList())
                                errors.append(
                                    'Please enter the Fixed amount for the Fixed Deposit A/C.')
                                raise forms.ValidationError(errors)
                        else:
                            errors = self._errors.setdefault(
                                'fixeddeposit_amount', ErrorList())
                    ...
                    errors = self._errors.setdefault(
                        'fixed_deposit_account_no', ErrorList())
                    errors.append(
                        'Please Create a Savings A/C first to store the Fixed amount for the Fixed Deposit A/C.')
                    raise forms.ValidationError(errors)
            ...
            if self.cleaned_data.get("fixeddeposit_amount"):
                errors = self._errors.setdefault(
                    'fixeddeposit_amount', ErrorList())
                errors.append(
                    'Please, select the fixed deposit before you enter the amount or clear the amount.')
                raise forms.ValidationError(errors)

        if self.cleaned_data.get('recurring_deposit_account_no'):
            recurring_deposit_account = RecurringDeposits.objects.filter(
                reccuring_deposit_number=self.cleaned_data.get(
                    'recurring_deposit_account_no')
            ).first()
            if recurring_deposit_account:
                savings_account = SavingsAccount.objects.filter(
                    client=recurring_deposit_account.client)
                if savings_account:
                    self.savings_account = savings_account.last()
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    errors = self._errors.setdefault(
                                        'message1', ErrorList())
                                    errors.append(
                                        'Entered recurring amount is not equal to the actual amount.')
                                    raise forms.ValidationError(errors)
                            else:
                                raise forms.ValidationError(
                                    'You have exceeded the recurring deposit reciepts adding to this A/C.')
                        else:
                            errors = self._errors.setdefault(
                                'recurringdeposit_amount', ErrorList())
                            errors.append(
                                'Please enter the Recurring amount for the Recurring Deposit A/C.')
                            raise forms.ValidationError(errors)
                else:
                    errors = self._errors.setdefault(
                        'recurring_deposit_account_no', ErrorList())
                    errors.append(
                        'Please Create a Savings A/C first to store the Recurring amount for the Recurring Deposit A/C.')
                    raise forms.ValidationError(errors)
            ...
            if self.cleaned_data.get("recurringdeposit_amount"):
                errors = self._errors.setdefault(
                    'recurringdeposit_amount', ErrorList())
                errors.append(
                    'Please, select the recurring deposit before you enter the amount or clear the amount.')
                raise forms.ValidationError(errors)
...
class PaymentForm(forms.ModelForm):
    ...
    def clean_date(self):
        datestring_format = datetime.datetime.strptime(
            self.data.get("date"), "%m/%d/%Y").strftime("%Y-%m-%d")
        if self.data.get("payment_type") == "Loans":
            loan = None
            if self.data.get("member_loan_account_no"):
                loan = LoanAccount.objects.filter(
                    id=self.data.get("member_loan_account_no")).first()
            elif self.data.get("group_loan_account_no"):
                loan = LoanAccount.objects.filter(
                    id=self.data.get("group_loan_account_no")).first()
            if loan:
                if str(datestring_format) < str(loan.opening_date):
                    raise forms.ValidationError(
                        "Payment date should be greater than Loan Application date")
        return self.cleaned_data.get("date")

    def clean_voucher_number(self):
        voucher_number = self.cleaned_data.get("voucher_number")
        is_voucher_number_exist = Payments.objects.filter(
            voucher_number=voucher_number)
        if is_voucher_number_exist:
            raise forms.ValidationError(
                "Payslip with this Voucher number already exists.")
    ...
    def clean(self):
        ...
        if self.cleaned_data.get("payment_type") == "TravellingAllowance" or self.cleaned_data.get("payment_type") == "Paymentofsalary":
            if not self.cleaned_data.get("staff_username"):
                ...
                self.staff = User.objects.filter(
                    username__iexact=self.cleaned_data.get("staff_username")).first()
                if not self.staff:
                    raise forms.ValidationError(
            ...
            if self.cleaned_data.get("interest"):
                ...
                if d(self.cleaned_data.get("total_amount")) != \
                    ...
                    raise forms.ValidationError(
                        "Entered total amount is incorrect.")
            else:
            ...
            if not (self.cleaned_data.get("client_name") or self.cleaned_data.get('group_name')):
                raise forms.ValidationError(
                    "Please enter the Member First Name or Group Name")
                ...
                if self.cleaned_data.get("client_name") and self.cleaned_data.get('client_account_number'):
                    client_filter = Client.objects.filter(
                        first_name__iexact=self.cleaned_data.get(
                            "client_name"),
                        account_number=self.cleaned_data.get("client_account_number"))
                    if client_filter:
                        ...
                        if savings_account:
                            ...
                            if self.cleaned_data.get("amount"):
                                if d(savings_account.savings_balance) >= total_amount:
                                    ...
                                    if client_group:
                                        group_savings_account = SavingsAccount.objects.filter(
                                            group=client_group)
                ...
                if self.cleaned_data.get("group_account_number"):
                    ...
                    if group_filter:
                        ...
                        group_savings_account_filter = SavingsAccount.objects.filter(
                            group=group)
                        if group_savings_account_filter:
                            ...
                            if d(group_savings_account.savings_balance) >= total_amount:
                                ...
                                if savings_accounts_filter:
                                    deduct_amount = total_amount / \
                                        d(len(savings_accounts_filter))
                                    if savings_accounts_filter:
                                ...
                                raise forms.ValidationError(
                                    'Entered amount is higher than the savings amount.')
                        else:
                            raise forms.ValidationError(
                                'Savings A/C not existed with the entered group.')
                    else:
                        raise forms.ValidationError(
                            "Group with given details doesn't exist.")
                else:
                    raise forms.ValidationError(
                        "Please provide the group A/C number to proceed with Group Savings Withdrawal.")
            ...
            if (
                ...
                raise forms.ValidationError(
                    "Don't include group details while processing FixedWithdrawal")
            elif self.cleaned_data.get('client_name'):
                if not self.cleaned_data.get("client_account_number"):
                    raise forms.ValidationError(
                        "Please enter the Member Account number")
                        ...
                        if self.cleaned_data.get("fixed_deposit_account_no"):
                            ...
                            if self.fixed_deposit_account_filter:
                                ...
                                if self.fixed_deposit_account:
                                    ...
                                    year_days = 366 if calendar.isleap(
                                        current_date.year) else 365
                                    interest_charged = (
                                        fixed_deposit.fixed_deposit_amount * fixed_deposit.fixed_deposit_interest_rate) / (d(year_days) * 100)
                                    days_to_calculate = (
                                        current_date - fixed_deposit.deposited_date).days
                                    ...
                                    total_amount = fixed_deposit.fixed_deposit_amount + \
                                        calculated_interest_money_till_date
                                    if self.cleaned_data.get("amount"):
                                        if d(fixed_deposit_amount) > d(self.cleaned_data.get("amount")):
                                            raise forms.ValidationError(
                                                "Entered amount is less than the Member Fixed Deposit amount.")
                                        elif d(fixed_deposit_amount) < d(self.cleaned_data.get("amount")):
                                            raise forms.ValidationError(
                                                "Entered amount is greater than the Member Fixed Deposit amount.")
                                        elif d(fixed_deposit_amount) != d(self.cleaned_data.get("amount")):
                                            raise forms.ValidationError(
                                                "Entered amount is not equals to the Member Fixed Deposit amount.")
                                    if self.cleaned_data.get("interest"):
                                        ...
                                        raise forms.ValidationError(
                                            "Interest Amount field is required for Fixed Deposit Wirthdrawl.")
                            ...
                            raise forms.ValidationError(
                                "Member does not have Fixed Deposit Account to withdraw amount.")
                    else:
                        raise forms.ValidationError(
                            "Member does not exists with this First Name and A/C Number. Please enter correct details.")
            ...
            if (
                ...
                raise forms.ValidationError(
                    "Don't include group details while processing RecurringWithdrawal")
            elif self.cleaned_data.get('client_name'):
                if not self.cleaned_data.get("client_account_number"):
                    raise forms.ValidationError(
                        "Please enter the Member Account number")
                        ...
                        if self.cleaned_data.get("recurring_deposit_account_no"):
                            ...
                            if self.recurring_deposit_account_filter:
                                ...
                                if self.recurring_deposit_account:
                                    ...
                                    recurring_deposit_amount = d(
                                        recurring_deposit.recurring_deposit_amount) * recurring_deposit.number_of_payments
                                    ...
                                    year_days = 366 if calendar.isleap(
                                        current_date.year) else 365
                                    interest_charged = (
                                        recurring_deposit_amount * recurring_deposit.recurring_deposit_interest_rate) / (d(year_days) * 100)
                                    days_to_calculate = (
                                        current_date - recurring_deposit.deposited_date).days
                            ...
                            raise forms.ValidationError(
                                "Member does not have Recurring Deposits Account to withdraw amount.")
                    else:
                        raise forms.ValidationError(
                            "Member does not exists with this First Name and A/C Number. Please enter correct details.")
            ...
            if not (self.cleaned_data.get("group_name") or self.cleaned_data.get("client_name")):
                ...
                if not self.cleaned_data.get("group_account_number"):
                    raise forms.ValidationError(
                        "Please enter Group Account Number.")
                                                ...
                                                if d(loan_account.loan_amount) == d(self.cleaned_data.get("total_amount")):
                                                    ...
                                                    if clients_list:
                                                        if len(clients_list) == 0:
                                                            raise forms.ValidationError(
                                                                "Group does not contain members inorder to issue Loan.")
                                                    else:
                                                        raise forms.ValidationError(
                                                            "Group does not contain members inorder to issue Loan.")
                                                else:
                                                    raise forms.ValidationError(
                                                        "Amount is not equal to the applied loan amount.")
                                            else:
                                                raise forms.ValidationError(
                                                    "Entered total amount is not equal to amount.")
                                    else:
                                        raise forms.ValidationError(
                                            "Group Loan Account is not yet Approved.")
                                else:
                                    raise forms.ValidationError(
                                        "Group Loan Account is already Issued.")
                            else:
                                raise forms.ValidationError(
                                    "Group does not have any Loan with this Loan A/C Number.")
                    else:
                        raise forms.ValidationError(
                            "Group does not exists with this Name and A/C Number. Please enter correct details.")
            elif self.cleaned_data.get("client_name"):
                if (
                    self.cleaned_data.get('group_name') or self.cleaned_data.get('group_account_number') or
                    (self.cleaned_data.get('group_name')
                     and self.cleaned_data.get('group_account_number'))
                ):
                    raise forms.ValidationError(
                ...
                if not self.cleaned_data.get("client_account_number"):
                    ...
                    if member_filter:
                        ...
                        if not self.cleaned_data.get("member_loan_account_no"):
                            raise forms.ValidationError(
                                "Please enter the Member Loan Account Number.")
                        else:
                            loan_account_filter = LoanAccount.objects.filter(
                                id=self.cleaned_data.get("member_loan_account_no"))
                            if loan_account_filter:
                                loan_account = loan_account_filter.first()
                                if not Payments.objects.filter(loan_account=loan_account, client=client):
                                    if loan_account.status == "Approved":
                                        ...
                                        raise forms.ValidationError(
                                            "Loan account is not yet approved")
                                else:
                                    raise forms.ValidationError(
                                        "Loan account is already issued.")
                            else:
                                raise forms.ValidationError(
                                    "Client does not have any Loan with this Loan A/C Number.")
                    else:
                        raise forms.ValidationError(
                            "No Member exists with this First Name and A/C number. Please enter correct details.")
        ...
        if not instance.id:
            if (
                ...
                staff = User.objects.get(
                    username__iexact=self.cleaned_data.get("staff_username"))
                    ...
                    if self.cleaned_data.get("group_name"):
                        if self.cleaned_data.get("group_account_number"):
                            group = Group.objects.filter(
                                name__iexact=self.cleaned_data.get(
                                    "group_name"),
                                    ...
                                    if d(group_savings_account.savings_balance) >= total_amount:
                                        ...
                                        if savings_accounts_filter:
                                            deduct_amount = total_amount / \
                                                d(len(savings_accounts_filter))
                                            for savings_account in savings_accounts_filter:
                                                savings_account.savings_balance -= d(
                                                    deduct_amount)
                                                savings_account.total_withdrawals += d(
                                                    deduct_amount)
                                                savings_account.save()
                ...
                if self.cleaned_data.get("client_name") and not self.cleaned_data.get("group_name"):
                    if self.cleaned_data.get("client_account_number"):
                        ...
                        if client:
                            loan_account = LoanAccount.objects.filter(
                                id=self.cleaned_data.get("member_loan_account_no")).first()
                            if loan_account:
                                instance.client = client
                                instance.loan_account = loan_account
                                loan_account.loan_issued_date = datetime.datetime.now().date()
                        ...
                        if client:
                            if self.cleaned_data.get('fixed_deposit_account_no'):
                                ...
                                if fixed_deposits_account:
                                    ...
                                    fixed_deposits_account.total_withdrawal_amount_principle = self.cleaned_data.get(
                                        'total_amount')
                                    fixed_deposits_account.total_withdrawal_amount_interest = self.cleaned_data.get(
                                        'interest')
                        ...
                        if client:
                            if self.cleaned_data.get('recurring_deposit_account_no'):
                                ...
                                if recurring_deposits_account:
                                    ...
                                    recurring_deposits_account.total_withdrawal_amount_principle = self.cleaned_data.get(
                                        'total_amount')
                                    recurring_deposits_account.total_withdrawal_amount_interest = self.cleaned_data.get(
                                        'interest')
